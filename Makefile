...
# and packages the client. If a special Config.as was specified at
# the command line, it is removed here so that the default persists
# after the build.
#

hudsongraph: checkclient clientoutput clientconfig
	$(MXMLC) $(MXMLCOPTS)
	$(Z) $(OUTPUT)/$(CLIENTNAME)/$(BUILDTAG).tar.gz -C $(SWFDIR) $(SWFNAME) -C $(SWFTMPDIR) $(SWFTMPNAME) -C $(DEF_OUT) version.php
ifneq ($(strip $(CLIENTCONFIG)),)
	cp $(DEF_OUT)/Config.as client/FlexClient/DrawGraph/src/Config.as
endif
...
hudsontest: checkclient testclientoutput clientconfig
	$(MXMLC) $(MXMLCTESTOPTS)
...
#
# If a client config file is designated, copy it over the default Config.as
# before the client is built.
#

clientconfig: output
ifneq ($(strip $(CLIENTCONFIG)),)
	cp client/FlexClient/DrawGraph/src/Config.as $(DEF_OUT)/Config.as
	cp $(CLIENTCONFIG) client/FlexClient/DrawGraph/src/Config.as
endif
